...
void Model::Salary::userInputSalary(){
	...
	try {
		...
		setBaseSalary(std::stof(input("Enter Base Salary OR " + msg, salaryRegex, true).value_or("0"))); 
		setBonus(std::stof(input("Enter Bonus OR ", salaryRegex, true).value_or("0"))); 
...
bool Model::Salary::updateSalary() {
	try {
		...
		setId(std::stoi(input( "Enter the Eid to update Salary : ",idRegex).value_or("0")));
		...
		if (DB::Database::row == 0) {
			...
			while (check) {
				...
				case 1:
					setBaseSalary(std::stof(input("Enter Base Salary: ", salaryRegex).value_or("0")));  
					break;

				case 2:
					setBonus(std::stof(input("Enter Bonus: ", salaryRegex).value_or("0"))); 
					break;
